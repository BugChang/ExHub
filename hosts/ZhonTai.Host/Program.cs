        ...
        context.Services.AddTaskScheduler(DbKeys.TaskDb, options =>
        {
            ...
            options.ConfigureFreeSchedulerBuilder = freeSchedulerBuilder =>
            {
                ...
                .OnExecuting(task =>
                {
                    ...
                    if (task.Topic == TaskNames.SinoExpressSyncTask)
                    {
                        Task.Run(async () =>
                        {
                            using var scope = AppInfo.ServiceProvider.GetRequiredService<IServiceScopeFactory>()
                                .CreateScope();
                            var jobService = scope.ServiceProvider.GetRequiredService<IJobService>();
                            var remark = await jobService.SyncSinoExpressAsync();
                            task.Remark(remark);
                    ...
                    if (task.Topic == TaskNames.SinoProductSyncTask)
                    {
                        Task.Run(async () =>
                        {
                            using var scope = AppInfo.ServiceProvider.GetRequiredService<IServiceScopeFactory>()
                                .CreateScope();
                            var jobService = scope.ServiceProvider.GetRequiredService<IJobService>();
                            var remark = await jobService.SyncSinoProductAsync();
                            task.Remark(remark);
                    ...
                    if (task.Topic == TaskNames.SendLicenseExpirationMessageTask)
                    {
                        Task.Run(async () =>
                        {
                            using var scope = AppInfo.ServiceProvider.GetRequiredService<IServiceScopeFactory>()
                                .CreateScope();
                            var jobService = scope.ServiceProvider.GetRequiredService<IJobService>();
                            await jobService.SendLicenseExpirationMessageAsync();

                        }).Wait();
                    }

                    if (task.Topic == TaskNames.SyncDealerAddressTask)
                    {
                        Task.Run(async () =>
                        {
                            using var scope = AppInfo.ServiceProvider.GetRequiredService<IServiceScopeFactory>()
                                .CreateScope();
                            var jobService = scope.ServiceProvider.GetRequiredService<IJobService>();
                            await jobService.SyncDealerAddressAsync();
...
<p>告警内容：<br/>{taskLog.Exception}</p>"
        ...
        // 添加诺诺SDK
        context.Services.AddNuoNuoSdk(context.Configuration);
    },
