...
public class AuthService : BaseService, IAuthService, IDynamicApi
{
    ...
    private readonly IAzureAdService _azuADService;
    ...
    public string GetToken(AuthLoginOutput user)
    {
        ...
        var claims = new List<Claim>()
       {
            ...
            new Claim("SoldCode", user.SoldCode??""),
            new Claim("RoleNames", string.Join(",",user.RoleNames)),
        };
    ...
    public async Task<dynamic> LoginAsync(AuthLoginInput input)
    {
        ...
        #region 登录
        var user = await userRep.Select.IncludeMany(a => a.Roles).Where(a => a.UserName == input.UserName).ToOneAsync();
        var valid = user?.Id > 0;
        if (valid)
        {
        ...
        authLoginOutput.RoleNames = user.Roles.Select(a => a.Name).ToList();
        if (_appConfig.Value.Value.Tenant)
        {
    ...
    {
        return _azuADService.GetAuthorizeUserInfoUrl();
    }
    ...
    [AllowAnonymous]
    [NoOprationLog]
    public async Task<dynamic> AzureAdLoginAsync(string accessToken)
    {
        var userRep = _userRep.Value;
        ...
        var stopwatch = Stopwatch.StartNew();
        //通过accessToken获取AzureAD的用户信息
        var data = _azuADService.GetOidcUserinfoAsync(accessToken);
        if (data != null)
        {
            #region 登录
            var user = await userRep.Select.IncludeMany(a => a.Roles).Where(a => a.Email == data.email).ToOneAsync();
            var valid = user?.Id > 0;
